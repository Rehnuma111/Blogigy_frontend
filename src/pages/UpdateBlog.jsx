...
const UpdateBlog = () => {
  ...
  const firebaseUploadImg = async () => {
    ...
    try {
      ...
      setFirebaseBlogImgUrl(imageUrl); // ✅ This is already used for preview
      setFormData((prev) => ({
        ...prev,
        ...
        <form action="" className="flex flex-col w-10/12 gap-5">
          <div className="flex gap-5">
            <input
              ...
              value={formData && formData.blogTitle}
            />

            <select
              defaultValue={"Select category"}
              className={` outline-none py-2 rounded-md px-5 border ${
                theme === "dark" && "bg-gray-700 border border-gray-500"
              }`}
              required
              name="blogCategory"
              onChange={inputChangeHandle}
              value={formData && formData.blogCategory}
            >
            ...
            {formData && (
              <img
                src={
                  ...
                  (firebaseBlogImgUrl && firebaseBlogImgUrl)
                }
